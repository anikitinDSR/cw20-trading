...
docker compose cp osmosis/osmosis-init.sh osmosis:/home/osmosis-init.sh
docker compose exec osmosis sh /home/osmosis-init.sh
...
info "Send some tokens to it" # ---
RES=$(docker compose exec osmosis osmosisd tx bank send osmosis-user "${OSMOSIS_RELAYER_ADDRESS}" 10000000uosmo --chain-id osmosis -y --keyring-backend test --output json --fees 875uosmo)
assert_tx_successful "${RES}"
...
info "Open channel" # ---
docker compose exec hermes hermes create channel --yes --a-chain cheqd --b-chain osmosis --a-port transfer --b-port transfer --new-client-connection
...
PORT="transfer"
CHANNEL="channel-0"
docker compose exec osmosis osmosisd tx ibc-transfer transfer $PORT $CHANNEL "$CHEQD_USER_ADDRESS" 10000000000"${DENOM}" --from osmosis-user --chain-id osmosis --keyring-backend test -y --fees 875uosmo
sleep 30 # Wait for relayer
